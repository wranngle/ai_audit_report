...
export class LLMExecutor {
   ...
   * Uses 18000 max tokens to ensure complete HTML is returned without truncation
  ...
  async polishHTML(html) {
    ...
    console.log('Running final HTML polish pass (18000 max tokens)...');
    const startTime = Date.now();

    // Override max_tokens to 18000 for HTML polish - ensures no truncation
    const polishPrompt = { ...prompt, max_tokens: 18000 };

    // Prepare context with the full HTML
    const context = { full_html: html };

    // Call LLM with high token limit
    const result = await this.callLLM(polishPrompt, context);
    ...
    if (!polishedHtml.includes('<!DOCTYPE html>') || !polishedHtml.includes('</html>')) {
      console.warn(`HTML polish returned incomplete document (${polishedHtml.length} chars), using manual polish`);
      return this.manualPolishHTML(html);
    }
    ...
    console.log(`HTML polish complete in ${elapsed}ms: ${changes.length} improvements`);
