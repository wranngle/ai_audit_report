...
const DEFAULT_CONFIG = {
  ...
  cta: {
    link: "https://calendly.com/wranngle-systems",
    link_display: "calendly.com/wranngle-systems",
 ...
 */
function buildScorecardRows(measurements, maxRows = 6) {
  ...
  return limited.map(m => {
    ...
    // Build metrics array - only include target if one exists
    const metrics = [
      {
        label: "Your metric",
        value_display: m.value_display,
...
function buildFixes(intake, measurements, maxFixes = 4) {
  ...
  const items = activeGroups.map(([key, group], idx) => {
    ...
    const impactShare = relatedIds.length / criticalMeasurements.length;
    ...
    return {
      ...
      impact: {
        estimated_recovery: {
          amount: Math.round(totalBleed * 0.7 * impactShare),
          currency: "USD",
          display: `$${Math.round(totalBleed * 0.7 * impactShare).toLocaleString()}`
        },
      ...
      effort: {
        tier: idx === 0 ? "low" : "medium",
        estimated_hours_range: {
          min_hours: idx === 0 ? 4 : 8,
          most_likely_hours: idx === 0 ? 8 : 16,
          max_hours: idx === 0 ? 16 : 24
        },
        skills_required: ["automation", "API"]
      },
      turnaround: {
        label: idx === 0 ? "2-3 Days" : "3-5 Days",
        business_days_min: idx === 0 ? 2 : 3,
        business_days_max: idx === 0 ? 3 : 5
      },
      dependencies: [],
...
export function transform(intake, measurements, userConfig = {}) {
  ...
  return {
    ...
    document: {
      ...
      title: "[LLM_PLACEHOLDER: document_title]",
      subtitle: "Traffic Light Report",
