...
export class Pipeline {
  ...
  /**
   * Stage 4: Fill LLM placeholders
   */
  async fillNarratives(reportJson) {
    if (this.config.skipLLM) {
      this.log('Skipping LLM fill (skipLLM=true)');
      this.stats.stages.llm = { skipped: true };
      return reportJson;
    }

    this.log('Filling narrative fields with LLM...');
    ...
    const filledReport = await executor.fillPlaceholders(reportJson, {
      onProgress: (progress) => {
        this.log(`  [${progress.current}/${progress.total}] ${progress.name}`);
      }
    });

    // Stage 4b: Final polishing pass
    this.log('Polishing report...');
    const polishedReport = executor.polishReport(filledReport);
    const polishLog = executor.getPolishLog();

    const llmStats = executor.getStats();
    this.stats.stages.llm = {
      complete: true,
      promptsExecuted: llmStats.promptsExecuted,
      tokensUsed: llmStats.tokensUsed,
      approvalRequired: llmStats.approvalRequired.length,
      polishFixes: polishLog.length
    };

    // Save polish log for review
    this.stats.polishLog = polishLog;

    this.log(`LLM fill complete: ${llmStats.promptsExecuted} prompts, ${llmStats.tokensUsed} tokens`);
    if (polishLog.length > 0) {
      this.log(`Polishing applied ${polishLog.length} fixes`);
    }

    if (llmStats.approvalRequired.length > 0) {
      this.log(`${llmStats.approvalRequired.length} fields require human approval`, 'warn');
    }
  ...
  async run(intakePath, measurementsPath, outputPath, options = {}) {
    ...
    try {
      ...
      // Stage 4: LLM fill
      if (!options.skipLLM && !this.config.skipLLM) {
      ...
      // Stage 6: Render
      this.render(reportJson, outputPath);
  ...
  async extract(infoDumpPath, options = {}) {
    ...
    if (options.outputDir) {
      const intakePath = path.join(options.outputDir, 'intake_extracted.json');
      const measurementsPath = path.join(options.outputDir, 'measurements_extracted.json');
   ...
   * Info Dump → Extract → Transform → LLM Fill → Validate → Render
   */
...
`;
  ...
  const pipeline = new Pipeline({
    skipLLM: rest.includes('--skip-llm'),
    dryRun: rest.includes('--dry-run'),
    allowPlaceholders: rest.includes('--skip-llm'),
    verbose: true
  });
