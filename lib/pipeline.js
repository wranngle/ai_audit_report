...
export class Pipeline {
  ...
  /**
   * Stage 8: Final HTML polish pass (manual by default, LLM only if llmPolish option)
   * Manual polish: fast deterministic fixes
   * LLM polish: slower but more thorough (opt-in via --llm-polish)
   */
  async polishHTMLPass(html, outputPath, options = {}) {
    ...
    let polishedHtml, changes;
    if (options.llmPolish) {
      // LLM polish: slower but more thorough (opt-in)
      const result = await executor.polishHTML(html);
      polishedHtml = result.html;
      changes = result.changes;
    } else {
      // Manual polish: fast deterministic fixes (default)
      const result = executor.manualPolishHTML(html);
      polishedHtml = result.html;
      changes = result.changes;
    }
  ...
  async run(intakePath, measurementsPath, outputPath, options = {}) {
    ...
    try {
      ...
      // Stage 7: HTML polish pass (manual by default, LLM only if --llm-polish)
      if (!options.skipHtmlPolish) {
        await this.polishHTMLPass(html, outputPath, { llmPolish: options.llmPolish });
      }
  ...
  async generate(infoDumpPath, outputPath, options = {}) {
    ...
    try {
      ...
      // Stage 5: HTML polish pass (manual by default, LLM only if --llm-polish)
      if (!options.skipHtmlPolish) {
        await this.polishHTMLPass(html, finalOutputPath, { llmPolish: options.llmPolish });
      }
...
Options:
  ...
  --llm-polish   Use LLM for final polish (slower, default: manual polish)
  ...
  switch (command) {
    case 'generate':
      ...
      await pipeline.generate(paths[0], genOutput, {
        ...
        llmPolish: rest.includes('--llm-polish')
      });
      break;
